...
def traces_vs_step(num_walkers, num_params, chain, max_posterior_theta, theta_names, lower_bounds, upper_bounds):
    ...
    for i in range(6):
        ...
        ax.axhline(y=max_posterior_theta[i], color="green", linestyle="--")
...
def posterior_pdf(current_theta_pdf, theta_names, max_posterior_theta, theta_idx, lower_bounds, upper_bounds):
    ...
    ax.axvline(x=max_posterior_theta[theta_idx], color="green", linestyle="--")
...
def check_mcmc_results(file_name, burn_in=20):
    ...
    traces_vs_step(num_walkers, num_params, chain, max_posterior_theta,
                   theta_names, lower_bounds, upper_bounds)

    for theta_idx in range(num_params):
        current_theta_pdf = flat_chain[:, theta_idx]
        posterior_pdf(current_theta_pdf, theta_names, max_posterior_theta, theta_idx, lower_bounds, upper_bounds)
