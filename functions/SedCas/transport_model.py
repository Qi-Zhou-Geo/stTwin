...
def trans_model(large_ls_t, small_ls_t, hyd, Q_theta, s_max, d_h, hs_theta, area, method, ls_trigger,
                rainfall_triggered_ls_theta, initial_hs_storage=0, initial_ch_storage=0, **kwargs):
    '''
    ...
    for i in range(1, t):
        ...
        # concept: from each landslide a fraction defined by the d_h parameter is redeposited in the channel storage, the rest of the landslide goes directly into the channel
        redep = ls[i] * d_h  # redeposition according to redeposition rate
    ...
    # output is debris flow when 1) the volume is larger than mindf
    # and 2) the sediment concentration is larger than smax_nodf
