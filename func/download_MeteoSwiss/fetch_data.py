...
def map_meta_data(time_resolution):
    ...
    if time_resolution == "10 minutes":
        ...
        meta_data = {
            "station": "station_abbr",  # Station name
            "timestamp [UTC+0]": "reference_timestamp",  # time stamp
            f"precipitation [mm per time_step]": "rre150d0",  # Precipitation; daily total 6 UTC - 6 UTC following day, unit by mm/h
            "temperature [degree]": "tre200d0",  # Air temperature at 5 cm above grass; daily mean, unit by degree C
            "sun radiation [W per squared m]": "gre000d0"  # Global radiation; daily mean, unit by W/m^2
        }
    elif time_resolution == "Monthly":
        meta_data = {
            "station": "station_abbr",  # Station name
            "timestamp [UTC+0]": "reference_timestamp",  # time stamp
            f"precipitation [mm per time_step]": "rre150m0",  # Precipitation; monthly total, unit by mm/h
            "temperature [degree]": "tre200m0",  # Air temperature 2 m above ground; monthly mean, unit by degree C
            "sun radiation [W per squared m]": "gre000m0"  # Global radiation; monthly mean, unit by W/m^2
        }
    else:
        meta_data = None
...
def fetch_data4SedCas(station="mve", time_resolution="10 minutes", time_period="Today"):
    ...
    time_period_mapping = {"Today": "now",
                           "Current year": "recent",
                           "1980-1989": "historical_1980-1989",
    ...
    if time_resolution == "Monthly":
        csv_url = (f"https://data.geo.admin.ch/ch.meteoschweiz.ogd-smn/"
                   f"{station}/"
                   f"ogd-smn_{station}"
                   f"_{time_resolution_mapping.get(time_resolution)}"
                   f".csv")
    elif time_resolution == "Daily":
        csv_url = (f"https://data.geo.admin.ch/ch.meteoschweiz.ogd-smn/"
                   f"{station}/"
        ...
        csv_url = (f"https://data.geo.admin.ch/ch.meteoschweiz.ogd-smn/"
                   ...
                   f"ogd-smn_{station}"
                   f"_{time_resolution_mapping.get(time_resolution)}"
                   f"_{time_period_mapping.get(time_period)}.csv")
