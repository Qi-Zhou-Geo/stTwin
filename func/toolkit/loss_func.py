...
def get_mu_sigma(y_pred, start_time, end_time, ratio_of_faliure=0.5):
    ...
    if isinstance(aggregate_sed, (float, np.floating)):
        # incase the aggregate_sed is a signle value, like JH results
        aggregate_sed = np.array([aggregate_sed])
    ...
    if np.sum(non_zero_volume_id) > ratio_of_faliure * len(aggregate_sed):
        ...
        print(f"debris flow period: {start_time} to {end_time}:\n"
              f"num_of_zero_df_volume_prediction= {len(aggregate_sed) - np.sum(non_zero_volume_id)} out of {len(aggregate_sed)} \n")
        raise RuntimeError(
