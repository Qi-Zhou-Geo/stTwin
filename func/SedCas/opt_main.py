...
def run1time_sedcas(params_trial, num_iteration=200,
...
def load_climate_input4model(data_type):
    ...
    if data_type == "default":
    ...
    elif data_type == "2023-2026":
        climate_frocing_input = "climate_2023_2026_t.txt"
        resolution = 600  # unit is second
        data = pd.read_csv(f"{project_root}/data/SedCas_input/{climate_frocing_input}", header=0)
        ...
        temperature = data.iloc[:, 3].values
        sun_radiation = data.iloc[:, 4].values
    elif data_type == "2023-2026-whatif":
        climate_frocing_input = "climate_2023_2026_t_whatif.txt"
        resolution = 600  # unit is second
        data = pd.read_csv(f"{project_root}/data/SedCas_input/{climate_frocing_input}", header=0)
...
def objective(trial, params_trial):
    ...
    max_s2w = trial.suggest_float("max_s2w", 0.1, 1, log=True)
    ...
    # (4) run the model
    model.run_hydro()
    model.run_stochastic_simulations(seed=0, num_iteration=200, progress_bars=False)
...
def run_optimization(num_trials):
    ...
    run1time_sedcas(params_trial, progress_bars=False)
