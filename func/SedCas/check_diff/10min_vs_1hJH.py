...
ax.set_title(label=(
    f"{1 / tolerance} <= ratio <= {tolerance}\n"
    f"No Volume Prediction (JH): {num_no_prediction_JH}, Log-Posterior: {g_log_like_JH:.1f}\n"
...
plt.tight_layout()
plt.savefig(f"{current_dir}/tolerance={tolerance}.png", dpi=600)  # , transparent=True
plt.show()
plt.close(fig=fig)
