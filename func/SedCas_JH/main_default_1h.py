...
from func.SedCas_JH.SedCas import SedCas
...
sed_container, sed_output = model.run_stochastic_simulations(seed=0, num_iteration=100)

# update the attrs if the xr is 2024 version
template_sed_container = model._create_sed_dataset(num_iteration=1)
for var in sed_container.data_vars:
    sed_container[var].attrs = template_sed_container[var].attrs.copy()
    sed_output[f"{var}_Q1"].attrs = template_sed_container[var].attrs.copy()
    sed_output[f"{var}_Q50"].attrs = template_sed_container[var].attrs.copy()
    sed_output[f"{var}_Q99"].attrs = template_sed_container[var].attrs.copy()

# (5-1) visualize
time_coord = "time_str"
t1, t2 = "2000-01-01T00:00:00", "2018-01-01T00:00:00"
...
sed_output = sed_output.isel(time=mask)
...
sed_container.to_netcdf(f"sed_container_{t1[:4]}_{t2[:4]}.nc")
