#!/bin/bash
#SBATCH -t 00:05:00                # time limit: (D-HH:MM:SS)
#SBATCH --job-name=statistic       # job name, "Qi_run"

#SBATCH --ntasks=1
#SBATCH --array=1-540               # job array id
#SBATCH --cpus-per-task=1           # reuest 1 cpu to run in parallel

#SBATCH --mem-per-cpu=1G		    # Memory Request (per CPU; can use on GLIC)
...
srun python statistic_total_yield.py --scenario_idx "${scenario_idx}"
