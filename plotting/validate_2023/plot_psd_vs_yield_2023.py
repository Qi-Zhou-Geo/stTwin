...
# __modification time__ = Last modified: 2026-07-08T16:29:12
...
gs = gridspec.GridSpec(5, 1, height_ratios=[10, 10, 10, 10, 0.5])
...
id3 = np.where(time_str=="2023-10-24T12:00:00")[0][0]
id4 = np.where(time_str=="2023-10-24T15:00:00")[0][0] + 1
print(f"np.sum(df.iloc[id3:id4, 2])={np.sum(df.iloc[id3:id4, 2]):.2f}")
print(f"np.argmax(df.iloc[id3:id4, 2])={np.argmax(df.iloc[id3:id4, 2])}")
...
rewrite_x_ticks(ax=ax_twin, data_start=t1, data_end=t2, data_sps=1/600, x_interval=4)
...
ax.set_ylim(0, 1.5 * 1e3)
rewrite_x_ticks(ax=ax, data_start=t1, data_end=t2, data_sps=1/600, x_interval=4)
...
st_path = Path(project_root) / f"data/open_access/qz2023/cooked_ILL12_2023-10-24.mseed"
st = read(st_path)
st = st.trim(UTCDateTime(t1), UTCDateTime(t2))
waveform_plot(ax, st, x_interval=4)
...
# region
ax = plt.subplot(gs[3])
ax.set_title(label=f"(d)", loc="left", fontsize=7, fontweight='bold')
...
pro_path = Path(project_root) / f"data/open_access/qz2023/model_prediction_2023-10-24.txt"
...
ax, data_sps = psd_plot(fig, ax, cbar_ax, st, fix_colorbar=True, per_lap=0.5, wlen=60, x_interval=4)
...
png_path = Path(current_dir) / f"seismic_vs_yield_2023.png"
