...
sed_output = xr.open_dataset(f"{project_root}/functions/SedCas_bo/1h/sed_output_2004_2018.nc")
...
plt.tight_layout()
plt.savefig(f"{current_dir}/error_ratio_BO.png", dpi=600)  # , transparent=True
plt.show()
plt.close(fig=fig)


# calculate the likehood loss
sed_output = xr.open_dataset(f"{project_root}/functions/SedCas_bo/1h/sed_container_2004_2018.nc")
